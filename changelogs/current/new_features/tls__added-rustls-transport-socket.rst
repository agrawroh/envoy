...
ALPN / SAN overrides, SDS-managed certificate rotation, OCSP revocation, and fine-grained
``TlsParameters`` beyond min/max protocol version are not yet supported. Additionally,
``Network::Connection::ssl()`` returns ``nullptr`` for this socket,
