...
are relayed in both directions. The upstream ``CONNECT`` waits for the upstream HTTP/3 SETTINGS so
the session is negotiated, and sessions are bounded per connection. A route opts in with an
