 ...
 */
#define ALL_MYSQL_CONNECT_PROXY_STATS(COUNTER, GAUGE)                                              \
...
/**
 * MySQL proxy stats for on-demand. These stats are generated only if the MySQL proxy enables on
 * demand.
 */
#define ON_DEMAND_MYSQL_CONNECT_PROXY_STATS(COUNTER)                                               \
 ...
 */
struct MySQLConnectProxyStats {
  ALL_MYSQL_CONNECT_PROXY_STATS(GENERATE_COUNTER_STRUCT, GENERATE_GAUGE_STRUCT)
};

/**
 * Struct definition for on-demand related MySQL proxy stats. @see stats_macros.h
 * These stats are available if and only if the MySQL proxy enables on-demand.
 * Note that these stats has the same prefix as `MySQLConnectProxyStats`.
 */
struct OnDemandStats {
  ON_DEMAND_MYSQL_CONNECT_PROXY_STATS(GENERATE_COUNTER_STRUCT)
};
...
using TunnelingConfig =
    envoy::extensions::filters::network::mysql_connect_proxy::v3::MySQLConnectProxy_TunnelingConfig;
...
class TunnelingConfigHelperImpl : public TcpProxy::TunnelingConfigHelper,
                                  protected Logger::Loggable<Logger::Id::filter> {
public:
  TunnelingConfigHelperImpl(const envoy::extensions::filters::network::mysql_connect_proxy::v3::
...
public:
  OnDemandConfig(const envoy::extensions::filters::network::mysql_connect_proxy::v3::
                     MySQLConnectProxy_OnDemand& on_demand_message,
...
public:
  ...
  public:
    SharedConfig(
        const envoy::extensions::filters::network::mysql_connect_proxy::v3::MySQLConnectProxy&
            config,
        Server::Configuration::FactoryContext& context);
    const MySQLConnectProxyStats& stats() { return stats_; }
    ...
    TcpProxy::TunnelingConfigHelperOptConstRef tunnelingConfigHelper() {
  ...
  private:
    static MySQLConnectProxyStats generateStats(Stats::Scope& scope);
    ...
    const MySQLConnectProxyStats stats_;
    ...
    std::unique_ptr<TcpProxy::TunnelingConfigHelper> tunneling_config_helper_;
  ...
  const MySQLConnectProxyStats& stats() { return shared_config_->stats(); }
...
/**
 * Per-connection MySQL Proxy Cluster configuration.
 */
...
// connection has been closed. The MySQLConnectProxy is destroyed when the downstream connection is
// closed, so handling the upstream connection here allows it to finish draining or timeout.
