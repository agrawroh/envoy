...
public:
  ...
  Network::ListenerFilterFactoryCb createListenerFilterFactoryFromProto(
    ...
    MySQLInspector::ConfigSharedPtr config(
        std::make_shared<MySQLInspector::Config>(context.scope()));
    return
        [listener_filter_matcher, config](Network::ListenerFilterManager& filter_manager) -> void {
          filter_manager.addAcceptFilter(listener_filter_matcher,
                                         std::make_unique<MySQLInspector::Filter>(config));
        };
  }

  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<
        envoy::extensions::filters::listener::mysql_inspector::v3::MySQLInspector>();
  }
 ...
 */
REGISTER_FACTORY(MySqlInspectorConfigFactory,
                 Server::Configuration::NamedListenerFilterConfigFactory){
    "envoy.listener.mysql_inspector"};
