...
// Test read() method with real socket pairs to test RPING handling.
TEST_F(DownstreamReverseConnectionIOHandleTest, ReadRpingEchoScenarios) {
  ...
  // Complete RPING message - should be echoed and drained.
  {
  ...
  // RPING + application data - echo RPING, keep app data, disable echo.
  {
...
TEST_F(DownstreamReverseConnectionIOHandleTest, ReadPartialDataAndStateTransitions) {
  ...
  // Test partial RPING - should pass through and wait for more data.
  {
    int fds[2];
    ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, fds), 0);
  ...
  // Test non-RPING data - should disable echo permanently.
  {
    int fds[2];
    ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, fds), 0);
...
TEST_F(DownstreamReverseConnectionIOHandleTest, ReadEchoDisabledAndErrorHandling) {
  ...
  // Test that after echo is disabled, RPING passes through without processing.
  {
    int fds[2];
    ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, fds), 0);
  ...
  // Test EOF scenario - close the write end.
  {
    int fds[2];
    ASSERT_EQ(socketpair(AF_UNIX, SOCK_STREAM, 0, fds), 0);
