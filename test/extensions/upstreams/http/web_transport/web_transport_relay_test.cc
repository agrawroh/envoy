...
// A finished relay is reclaimed when the next stream is relayed, so a peer that opens and closes
// streams cannot grow the relay list without bound.
TEST_F(WebTransportRelayTest, ReclaimsFinishedStreamRelays) {
  FakeWebTransportStream mirror1(true);
  upstream_.next_outgoing_ = &mirror1;
  WebTransportRelay relay(downstream_, upstream_, callbacks_);

  FakeWebTransportStream incoming1(true);
  downstream_.callbacks_->onWebTransportStreamIncoming(incoming1, true);
  ASSERT_NE(nullptr, incoming1.callbacks_);

  // Reset the first stream so its relay is finished.
  incoming1.callbacks_->onWebTransportStreamReset(0);

  // Relaying a new stream reclaims the finished relay, detaching the first stream's callbacks.
  FakeWebTransportStream mirror2(true);
  upstream_.next_outgoing_ = &mirror2;
  FakeWebTransportStream incoming2(true);
  downstream_.callbacks_->onWebTransportStreamIncoming(incoming2, true);

  EXPECT_EQ(nullptr, incoming1.callbacks_);
  EXPECT_EQ(nullptr, mirror1.callbacks_);
  EXPECT_NE(nullptr, incoming2.callbacks_);
}
