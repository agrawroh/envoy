...
fn my_new_bootstrap_extension_config_fn(
  ...
  // Init completion is deferred to on_scheduled so initialize() blocks until secret lifecycle is
  // enabled, mirroring the cluster and listener lifecycle tests.
...
impl BootstrapExtensionConfig for MyBootstrapExtensionConfig {
  ...
  fn on_scheduled(
    ...
    if event_id == ENABLE_SECRET_LIFECYCLE_EVENT {
      // Enabling replays the secrets that are already active, so on_secret_add_or_update fires for
