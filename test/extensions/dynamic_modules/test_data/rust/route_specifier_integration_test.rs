  ...
  match name {
    ...
    b"cluster-host-count" => {
      let cluster_name = ctx
        ...
        .unwrap_or_default();
      ctx.get_cluster_host_count(&cluster_name, 0).map_or_else(
        || ABSENT.to_owned(),
...
impl RouteSpecifierConfig for TestRouteSpecifierConfig {
  fn on_route(&self, ctx: &mut RouteSpecifierContext) -> RouteDecision {
    ...
    if let Some(name) = ctx.get_request_header("x-override") {
      // Tests also pass names that are not declared, so a miss is expected here, and the
      // declaration getter reports the same answer without recording anything.
      let name = buffer_to_string(name);
      if self.envoy_config.has_route_action_override(&name) {
