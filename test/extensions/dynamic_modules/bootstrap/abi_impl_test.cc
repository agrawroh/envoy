...
// When ``timer_delete`` is called from a background thread, the wrapped Envoy timer is
// destroyed on the main thread dispatcher. Destroying the wrapper off-thread would deregister
// the timer from the dispatcher's timer list from a non-dispatcher thread.
TEST_F(BootstrapAbiImplTest, TimerDeleteFromBackgroundThreadPostsToMainDispatcher) {
  ...
  // Spawn a worker thread and call ``timer_delete`` from there. On non main/test threads the
  // implementation must post the deletion to the dispatcher rather than destroy the wrapper
  // inline.
  ...
  ASSERT_TRUE(captured_cb) << "timer_delete must post deletion to the main dispatcher when "
                              "invoked from a background thread";
  // Running the posted callback completes the deferred deletion.
  captured_cb();
}
