...
pub trait ListenerFilter<ELF: EnvoyListenerFilter> {
  ...
  /// This is called when the new event is scheduled via the [`EnvoyListenerFilterScheduler::commit`]
  /// for this [`ListenerFilter`].
  ///
  /// * `envoy_filter` can be used to interact with the underlying Envoy filter object.
  /// * `event_id` is the ID of the event that was scheduled with
...
/// [`EnvoyListenerFilterScheduler::commit`] with an event id, eventually [`ListenerFilter::on_scheduled`]
/// is called with the same event id on the worker thread where the listener filter is running, IF
/// the filter is still alive by the time the event is committed.
///
/// Since this is primarily designed to be used from a different thread than the one
