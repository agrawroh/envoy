...
impl From<PostIoAction> for abi::envoy_dynamic_module_type_transport_socket_post_io_action {
  fn from(value: PostIoAction) -> Self {
    match value {
      PostIoAction::KeepOpen => abi::envoy_dynamic_module_type_transport_socket_post_io_action::KeepOpen,
...
impl From<abi::envoy_dynamic_module_type_transport_socket_post_io_action> for PostIoAction {
  fn from(value: abi::envoy_dynamic_module_type_transport_socket_post_io_action) -> Self {
    match value {
      abi::envoy_dynamic_module_type_transport_socket_post_io_action::KeepOpen => PostIoAction::KeepOpen,
...
impl From<ConnectionEvent> for abi::envoy_dynamic_module_type_network_connection_event {
  fn from(value: ConnectionEvent) -> Self {
    match value {
      ConnectionEvent::RemoteClose => abi::envoy_dynamic_module_type_network_connection_event::RemoteClose,
      ConnectionEvent::LocalClose => abi::envoy_dynamic_module_type_network_connection_event::LocalClose,
      ConnectionEvent::Connected => abi::envoy_dynamic_module_type_network_connection_event::Connected,
...
impl From<abi::envoy_dynamic_module_type_network_connection_event> for ConnectionEvent {
  fn from(value: abi::envoy_dynamic_module_type_network_connection_event) -> Self {
    match value {
      abi::envoy_dynamic_module_type_network_connection_event::RemoteClose => ConnectionEvent::RemoteClose,
      abi::envoy_dynamic_module_type_network_connection_event::LocalClose => ConnectionEvent::LocalClose,
      abi::envoy_dynamic_module_type_network_connection_event::Connected => ConnectionEvent::Connected,
      abi::envoy_dynamic_module_type_network_connection_event::ConnectedZeroRtt => {
        ConnectionEvent::ConnectedZeroRtt
      },
    }
...
pub trait EnvoyTransportSocket {
  ...
  fn read_buffer_drain(&self, length: usize) -> bool;
  /// Appends `data` to the read buffer.
  fn read_buffer_add(&self, data: &[u8]) -> bool;
  ...
  fn write_buffer_drain(&self, length: usize) -> bool;
  ...
  /// Requests that a future event-loop iteration schedules a read. The `readable` hint is reserved
  /// for future ABI use and is ignored by the current Envoy callback signature.
...
impl EnvoyTransportSocket for EnvoyTransportSocketImpl {
  ...
  fn io_handle_fd(&self, io_handle: *mut c_void) -> Option<i32> {
    let fd =
      unsafe { abi::envoy_dynamic_module_callback_transport_socket_io_handle_fd(io_handle) };
  ...
  fn should_drain_read_buffer(&self) -> bool {
    unsafe { abi::envoy_dynamic_module_callback_transport_socket_should_drain_read_buffer(self.raw) }
  }
...
pub type NewTransportSocketFactoryConfigFunction<ETS> = fn(
  name: &str,
  config: &[u8],
  is_upstream: bool,
...
thread_local! {
  static GET_PROTOCOL_BUF: RefCell<Vec<u8>> = RefCell::new(Vec::new());
  static GET_FAILURE_REASON_BUF: RefCell<Vec<u8>> = RefCell::new(Vec::new());
}
...
fn fill_protocol_buffer_out(dst: *mut abi::envoy_dynamic_module_type_module_buffer, value: &str) {
  if dst.is_null() {
    return;
  }
  GET_PROTOCOL_BUF.with(|cell| {
...
fn fill_failure_reason_buffer_out(dst: *mut abi::envoy_dynamic_module_type_module_buffer, value: &str) {
  if dst.is_null() {
    return;
  }
  GET_FAILURE_REASON_BUF.with(|cell| {
    let mut buf = cell.borrow_mut();
    buf.clear();
    buf.extend_from_slice(value.as_bytes());
    let filled = bytes_to_module_buffer(buf.as_slice());
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_factory_config_new(
  factory_config_envoy_ptr: abi::envoy_dynamic_module_type_transport_socket_factory_config_envoy_ptr,
  ...
  catch_unwind(AssertUnwindSafe(|| {
    let _ = factory_config_envoy_ptr;
    let name_bytes =
    ...
    let Some(new_config_fn) = crate::NEW_TRANSPORT_SOCKET_FACTORY_CONFIG_FUNCTION.get() else {
      crate::envoy_log_error!(
        "transport socket factory config function not registered; \
         ensure declare_all_init_functions! includes transport_socket."
      );
      return std::ptr::null();
    };
  ...
  .unwrap_or_else(|panic| {
    log_panic("envoy_dynamic_module_on_transport_socket_factory_config_new", panic);
    std::ptr::null()
  })
}
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_factory_config_destroy(
  ...
  let _ = catch_unwind(AssertUnwindSafe(|| {
    drop_wrapped_c_void_ptr!(factory_config_ptr, TransportSocketFactoryConfig<EnvoyTransportSocketImpl>);
  }))
  .map_err(|panic| {
    log_panic(
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_new(
  ...
  catch_unwind(AssertUnwindSafe(|| {
    let config = factory_config_ptr as *const *const dyn TransportSocketFactoryConfig<EnvoyTransportSocketImpl>;
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_set_callbacks(
  ...
  .map_err(|panic| {
    log_panic("envoy_dynamic_module_on_transport_socket_set_callbacks", panic);
  });
}
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_on_connected(
  ...
  .map_err(|panic| {
    log_panic("envoy_dynamic_module_on_transport_socket_on_connected", panic);
  });
}
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_get_protocol(
  ...
  let _ = catch_unwind(AssertUnwindSafe(|| {
    ...
    fill_protocol_buffer_out(result, &s);
  }))
  .map_err(|panic| {
    log_panic("envoy_dynamic_module_on_transport_socket_get_protocol", panic);
  });
}
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_get_failure_reason(
  ...
  let _ = catch_unwind(AssertUnwindSafe(|| {
    ...
    fill_failure_reason_buffer_out(result, &s);
  }))
  .map_err(|panic| {
    log_panic("envoy_dynamic_module_on_transport_socket_get_failure_reason", panic);
  });
}
...
pub unsafe extern "C" fn envoy_dynamic_module_on_transport_socket_can_flush_close(
  ...
  .unwrap_or_else(|panic| {
    log_panic("envoy_dynamic_module_on_transport_socket_can_flush_close", panic);
    false
  })
}
