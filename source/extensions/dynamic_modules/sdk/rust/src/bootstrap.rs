...
pub enum ActiveResourceKind {
  /// Filter chains of the active listeners, both inline and FCDS delivered.
  FilterChain,
  ...
  /// Dynamic (SDS) TLS certificate secrets whose material has been delivered.
  Secret,
}
...
pub trait EnvoyBootstrapExtensionConfig {
  ...
  /// Enable secret lifecycle event notifications. When enabled, the module receives
  /// [`BootstrapExtensionConfig::on_secret_add_or_update`] and
  /// [`BootstrapExtensionConfig::on_secret_removal`] when a dynamic TLS certificate secret becomes
  /// active, is rotated, or is removed. Secrets are independent xDS resources, so these fire
  /// without any cluster or listener being re-pushed, and the ones already active are replayed
  /// before this returns.
  ///
  /// This must be called on the main thread, typically during or after `on_server_initialized`,
  ...
  /// Returns the names of the config objects Envoy currently has active for `kind`, so a module can
  /// reconcile its own view against what Envoy has committed. A name repeats when two objects of
  /// that kind carry it. Empty before `on_server_initialized`.
  ///
  /// This must be called on the main thread.
...
pub trait BootstrapExtensionConfig: Send + Sync {
  ...
  ///
  /// This is only called if the module has opted in via
  /// [`EnvoyBootstrapExtensionConfig::enable_secret_lifecycle`]. It is invoked on the main thread.
  ///
  /// * `envoy_extension_config` can be used to interact with the underlying Envoy config object.
  /// * `secret_name` is the name of the secret that became active or was rotated.
...
impl EnvoyBootstrapExtensionConfig for EnvoyBootstrapExtensionConfigImpl {
  ...
  fn active_resource_names(&self, kind: ActiveResourceKind) -> Vec<String> {
    ...
    let abi_kind = match kind {
      ActiveResourceKind::FilterChain => AbiKind::FilterChain,
      ActiveResourceKind::Cluster => AbiKind::Cluster,
      ActiveResourceKind::TransportSocketMatch => AbiKind::TransportSocketMatch,
      ActiveResourceKind::Secret => AbiKind::Secret,
    };
