 ...
 * envoy_dynamic_module_callback_route_specifier_config_* callbacks that read the specifier
 * configuration and the ones that define metrics may only be called from inside this hook.
 *
 ...
 * route templates the route specifier configuration declares. This may only be called from inside
 * envoy_dynamic_module_on_route_specifier_config_new.
 *
 * @param config_envoy_ptr is the pointer to the route specifier configuration.
 * @return the number of route templates, or zero when called outside the config hook.
 */
 ...
 * get the identifier of a route template by index, in configuration order. This may only be called
 * from inside envoy_dynamic_module_on_route_specifier_config_new.
 *
 * @param config_envoy_ptr is the pointer to the route specifier configuration.
 ...
 * using weighted clusters, is a route entry. This may only be called from inside
 * envoy_dynamic_module_on_route_specifier_config_new.
 *
 * @param config_envoy_ptr is the pointer to the route specifier configuration.
 ...
 * module to check whether a route action override is declared. This may only be called from inside
 * envoy_dynamic_module_on_route_specifier_config_new.
 *
 * @param config_envoy_ptr is the pointer to the route specifier configuration.
 ...
 * check whether the route specifier runs in shadow mode. This may only be called from inside
 * envoy_dynamic_module_on_route_specifier_config_new.
 *
 * @param config_envoy_ptr is the pointer to the route specifier configuration.
 ...
 *
 * @param context_envoy_ptr is the pointer to the route decision context.
 * @param result is the output buffer for the location. The buffer is owned by Envoy and is valid
 * until the end of the current event hook. The location is built once per hook and kept in storage
 * of its own, so repeated calls return the same buffer.
 * @return true if the resolved route answers the request directly, false otherwise.
 */
 ...
 * SelectTemplate decision uses. Envoy evaluates the match of the template against the request once
 * the hook returns, like it does for a configured route, and applies the failure policy when the
 * match does not hold.
 *
 * @param context_envoy_ptr is the pointer to the route decision context.
 * @param template_id is the identifier of the template. The buffer is owned by the module.
 ...
 * metadata match criteria, the request mirroring policies and the hash policy of the route with the
 * ones the override builds, for the properties that it sets.
 *
 * @param context_envoy_ptr is the pointer to the route decision context.
 * @param name is the name of the entry in the route_action_overrides map. The buffer is owned by
 * the module.
