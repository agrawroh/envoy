...
os_fd_t KtlsSocketHandleImpl::fdDoNotUse() const {
  return io_handle_->fdDoNotUse();
}

Api::IoCallUint64Result KtlsSocketHandleImpl::close() {
  return io_handle_->close();
}

bool KtlsSocketHandleImpl::isOpen() const {
...
Api::IoCallUint64Result KtlsSocketHandleImpl::writev(const Buffer::RawSlice* slices, uint64_t num_slice) {
...
Api::IoCallUint64Result KtlsSocketHandleImpl::writev(uint64_t num_slice, Buffer::RawSliceData* slice_data) {
  return io_handle_->writev(num_slice, slice_data);
}

Api::IoCallUint64Result KtlsSocketHandleImpl::send(const Buffer::RawSlice* slices, uint64_t num_slice,
                                                  int flags) {
  return io_handle_->send(slices, num_slice, flags);
}

Api::IoCallUint64Result KtlsSocketHandleImpl::sendmsg(const Buffer::RawSlice* slices, uint64_t num_slice,
                                                     int flags,
...
Api::IoCallUint64Result KtlsSocketHandleImpl::recvmmsg(RawSliceArrays& slices, uint32_t* self_port,
                                                      Network::Address::InstancePtr* peer_address) {
  return io_handle_->recvmmsg(slices, self_port, peer_address);
}

bool KtlsSocketHandleImpl::supportsMmsg() const {
  return io_handle_->supportsMmsg();
}

bool KtlsSocketHandleImpl::supportsUdpGro() const {
  return io_handle_->supportsUdpGro();
}
