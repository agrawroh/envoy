...
Envoy::Network::FilterFactoryCb ReverseConnectionConfigFactory::createFilterFactoryFromProtoTyped(
  ...
  const uint32_t max_connections = proto_config.has_max_connections_per_cluster()
                                       ? proto_config.max_connections_per_cluster().value()
                                       : 5;
  ...
  return [&context, stat_prefix, cluster_name, max_connections, connection_timeout,
          debug_logging](Envoy::Network::FilterManager& filter_manager) -> void {
    auto filter = std::make_shared<ReverseConnectionNetworkFilter>(
        stat_prefix, cluster_name, max_connections, connection_timeout, debug_logging,
