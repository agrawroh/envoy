...
/**
 * Terminal network filter for upstream Envoy that handles reverse connection handoff.
 *
 * Key responsibilities (yanavlasov's approach):
 * 1. Read cluster identification from incoming reverse connection
 * 2. Duplicate the socket descriptor via dup()
 * 3. Hand off duplicated descriptor to appropriate cluster's socket interface
 * 4. Close original connection after handoff
 *
...
private:
  ...
  /**
   * Close the original connection after handoff.
   */
