...
int ListenerStreamInfoWrapper::luaListenerDynamicMetadata(lua_State* state) {
  if (listener_dynamic_metadata_wrapper_.get() != nullptr) {
    listener_dynamic_metadata_wrapper_.pushStack();
  } else {
    listener_dynamic_metadata_wrapper_.reset(
        ListenerDynamicMetadataMapWrapper::create(state, *this), true);
  }
  return 1;
}
...
ListenerDynamicMetadataMapIterator::ListenerDynamicMetadataMapIterator(
    ListenerDynamicMetadataMapWrapper& parent)
    : parent_{parent}, current_{parent_.streamInfo().dynamicMetadata().filter_metadata().begin()} {}

StreamInfo::StreamInfo& DynamicMetadataMapWrapper::streamInfo() { return parent_.stream_info_; }
const StreamInfo::StreamInfo& ListenerDynamicMetadataMapWrapper::streamInfo() {
  return parent_.listener_stream_info_;
}
...
int ListenerDynamicMetadataMapIterator::luaListenerDynamicMetadataPairsIterator(lua_State* state) {
  if (current_ == parent_.streamInfo().dynamicMetadata().filter_metadata().end()) {
    parent_.iterator_.reset();
    return 0;
  }
...
int ListenerDynamicMetadataMapWrapper::luaListenerDynamicMetadataGet(lua_State* state) {
  const char* filter_name = luaL_checkstring(state, 2);
  const auto& metadata = streamInfo().dynamicMetadata().filter_metadata();
  const auto filter_it = metadata.find(filter_name);
  if (filter_it == metadata.end()) {
...
int ListenerDynamicMetadataMapWrapper::luaListenerDynamicMetadataPairs(lua_State* state) {
  if (iterator_.get() != nullptr) {
    luaL_error(state, "cannot create a second iterator before completing the first");
  }

  iterator_.reset(ListenerDynamicMetadataMapIterator::create(state, *this), true);
  lua_pushcclosure(
      state, ListenerDynamicMetadataMapIterator::static_luaListenerDynamicMetadataPairsIterator, 1);
  return 1;
}
