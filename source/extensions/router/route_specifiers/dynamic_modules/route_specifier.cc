...
newDynamicModuleRouteSpecifierConfig(const DynamicModuleRouteSpecifierProto& proto_config,
  ...
  config->template_ids_.reserve(proto_config.route_templates().size());
  ...
  config->in_config_new_.store(false, std::memory_order_release);
...
DynamicModuleRouteSpecifier::resolve(RouteSpecifierContext& context,
  ...
  case envoy_dynamic_module_type_route_specifier_decision_SelectTemplate: {
    ...
    Envoy::Router::RouteConstSharedPtr route = context.selected_template->route->match(
        context.headers, context.stream_info, context.random_value);
    if (route == nullptr) {
      return fail(envoy_dynamic_module_type_route_specifier_failure_TemplateMatchFailed);
    }
    Decision wrapped = wrap(std::move(route), context, status(Status::StopIteration));
    return wrapped.failure == envoy_dynamic_module_type_route_specifier_failure_None
               ? wrapped
               : fail(wrapped.failure);
  }
