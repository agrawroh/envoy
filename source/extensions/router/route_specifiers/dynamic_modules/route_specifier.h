...
public:
  ...
  const std::vector<std::string>& templateIds() const { return template_ids_; }
  ...
  // The route templates and the route action overrides can only be read while the in-module
  // configuration is being created, because the module is expected to resolve the names it uses
  // once rather than on every request.
  std::atomic<bool> in_config_new_{true};
...
private:
  ...
  // Const after construction so that the pointers routeTemplate() and routeActionOverride() hand
  // out stay valid.
...
struct RouteSpecifierContext {
  ...
  // Written at most once per hook, so that a buffer handed to the module stays valid until the hook
  // returns even when the module asks for it again.
  std::optional<std::string> input_redirect_location;
};
...
private:
  ...
  Decision resolve(RouteSpecifierContext& context,
                   envoy_dynamic_module_type_route_specifier_decision decision) const;
