...
// The configuration while it is being created, or nullptr once it has been, because the module is
// expected to resolve the names it uses once rather than on every request.
DynamicModuleRouteSpecifierConfig*
configurableConfig(envoy_dynamic_module_type_route_specifier_config_envoy_ptr ptr) {
  auto* config = static_cast<DynamicModuleRouteSpecifierConfig*>(ptr);
  return config->in_config_new_.load(std::memory_order_acquire) ? config : nullptr;
}
...
withGauge(envoy_dynamic_module_type_route_specifier_config_envoy_ptr config_envoy_ptr, size_t id,
  ...
  auto* config = static_cast<DynamicModuleRouteSpecifierConfig*>(config_envoy_ptr);
...
size_t envoy_dynamic_module_callback_route_specifier_config_get_template_count(
  ...
  auto* config = configurableConfig(config_envoy_ptr);
  return config != nullptr ? config->templateIds().size() : 0;
}
...
bool envoy_dynamic_module_callback_route_specifier_config_get_template_id(
  ...
  auto* config = configurableConfig(config_envoy_ptr);
  if (config == nullptr || index >= config->templateIds().size()) {
    return false;
  }
...
envoy_dynamic_module_callback_route_specifier_config_get_template_kind(
  ...
  const auto* route_template = config->routeTemplate(toStringView(template_id));
...
bool envoy_dynamic_module_callback_route_specifier_config_has_route_action_override(
  ...
  auto* config = configurableConfig(config_envoy_ptr);
  return config != nullptr && config->routeActionOverride(toStringView(name)) != nullptr;
}

bool envoy_dynamic_module_callback_route_specifier_config_is_shadow_mode(
    envoy_dynamic_module_type_route_specifier_config_envoy_ptr config_envoy_ptr) {
  auto* config = configurableConfig(config_envoy_ptr);
  return config != nullptr && config->shadow().has_value();
}
...
bool envoy_dynamic_module_callback_route_specifier_get_input_route_redirect_location(
    ...
    envoy_dynamic_module_type_envoy_buffer* result) {
  auto* context = routeSpecifierContext(context_envoy_ptr);
  if (context->input_route == nullptr || context->input_route->directResponseEntry() == nullptr) {
    return false;
  }
  // Built once per hook so that a buffer the module already holds is never invalidated.
  if (!context->input_redirect_location.has_value()) {
    context->input_redirect_location =
        context->input_route->directResponseEntry()->newUri(context->headers, context->stream_info);
  }
  setEnvoyBuffer(result, *context->input_redirect_location);
  return true;
}
