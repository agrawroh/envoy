...
// Opaque holder for the persistent (immer) cross-priority host map. Defined in upstream_impl.cc so
// the immer dependency stays out of this widely-included header.
...
public:
  ...
  // Selects the persistent (immer) cross-priority host map backing for this cluster in place of the
  // legacy flat copy-on-write map, trading the per-update O(N) copy for O(delta) updates. Must be
  // called on the main thread before the first host update; dynamic-modules clusters opt in through
  // the cluster ABI.
...
private:
  // Persistent (immer) backing, used when `use_persistent_cross_priority_host_map_` is set. The map
  // itself lives in the holder so immer stays confined to upstream_impl.cc.
  std::unique_ptr<PersistentHostMapHolder> persistent_host_map_;
