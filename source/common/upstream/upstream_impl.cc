...
// Holds the persistent (immer) cross-priority host map. Defined here so the immer type stays out of
// upstream_impl.h.
struct PersistentHostMapHolder {
...
MainPrioritySetImpl::MainPrioritySetImpl()
    : persistent_host_map_(std::make_unique<PersistentHostMapHolder>()) {}
...
void MainPrioritySetImpl::updateCrossPriorityHostMap(uint32_t priority,
  ...
  if (use_persistent) {
    // If the persistent backing was just selected, seed the persistent map from the flat backing so
    // the accumulated membership carries across the switch.
    if (!last_update_used_persistent_) {
      ...
      for (const auto& [address, host] : flat) {
        seeded = std::move(seeded).set(address, host);
      }
      persistent_host_map_->map = std::move(seeded);
    }

    PersistentHostMap updated = persistent_host_map_->map;
    for (const auto& host : hosts_removed) {
      const auto host_address = addressToString(host->address());
      const HostSharedPtr* existing_host = updated.find(host_address);
      ...
      if (existing_host != nullptr && (*existing_host)->priority() == priority) {
        updated = std::move(updated).erase(host_address);
      }
    }
    for (const auto& host : hosts_added) {
      // Match the flat path's `insert`, the first host wins on an address collision across
      // priorities, so the persistent backing is a pure performance lever with no behavior change.
      const auto host_address = addressToString(host->address());
      if (updated.find(host_address) == nullptr) {
        updated = std::move(updated).set(host_address, host);
      }
    }

    persistent_host_map_->map = std::move(updated);
    // Republish the lookup table so the cached wrapper is rebuilt only when the map changes.
    cross_priority_lookup_ = std::make_shared<PersistentHostLookupTable>(persistent_host_map_->map);
  ...
  if (last_update_used_persistent_) {
    ...
    for (const auto& [address, host] : persistent_host_map_->map) {
      seeded->insert({address, host});
    }
