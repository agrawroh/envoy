...
void AllocatorManager::configureBackgroundMemoryRelease() {
...
#endif
#if defined(GPERFTOOLS_TCMALLOC)
  // `gperftools` has no native release loop, so a dedicated thread requests `bytes_to_release_`
  // once per interval. It releases whole page-heap spans, so a given interval may release more,
  // less or no memory.
  ...
  // Block until the thread has armed the timer and entered its event loop, matching the idiom in
  // GuardDogImpl::start(). This keeps simulated-time tests deterministic.
  absl::Notification thread_started;
  tcmalloc_thread_ = api_.threadFactory().createThread(
      [this, &thread_started]() -> void {
        ENVOY_LOG_MISC(debug, "Started {}.", GPERFTOOLS_RELEASE_THREAD_ID);
        memory_release_timer_->enableTimer(memory_release_interval_msec_);
        tcmalloc_routine_dispatcher_->post([&thread_started]() { thread_started.Notify(); });
  ...
  ENVOY_LOG_MISC(info,
                 "Configured gperftools tcmalloc with background release rate: {} bytes every {} "
                 "milliseconds.",
