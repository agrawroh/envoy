#include "source/common/network/lc_trie_ip_list.h"
...
// static
absl::StatusOr<std::unique_ptr<LcTrieIpList>>
LcTrieIpList::create(const Protobuf::RepeatedPtrField<envoy::config::core::v3::CidrRange>& cidrs) {
  auto ip_list = std::unique_ptr<LcTrieIpList>(new LcTrieIpList(cidrs));
...
// static
absl::StatusOr<std::unique_ptr<LcTrieIpList>>
LcTrieIpList::create(const Protobuf::RepeatedPtrField<xds::core::v3::CidrRange>& cidrs) {
  auto ip_list = std::unique_ptr<LcTrieIpList>(new LcTrieIpList(cidrs));
  if (!ip_list->error().empty()) {
    return absl::InvalidArgumentError(ip_list->error());
  }
...
LcTrieIpList::LcTrieIpList(
    const Protobuf::RepeatedPtrField<envoy::config::core::v3::CidrRange>& cidrs) {
  initializeFromCidrs(cidrs);
}

LcTrieIpList::LcTrieIpList(const Protobuf::RepeatedPtrField<xds::core::v3::CidrRange>& cidrs) {
  initializeFromCidrs(cidrs);
}

template <typename CidrRangeType>
void LcTrieIpList::initializeFromCidrs(const Protobuf::RepeatedPtrField<CidrRangeType>& cidrs) {
  if (cidrs.empty()) {
    return;
  }
...
bool LcTrieIpList::contains(const Instance& address) const {
  ...
  if (!result.ok()) {
    ENVOY_LOG_MISC(debug, "LcTrieIpList lookup failed: {}", result.status().message());
    return false;
  }
...
absl::StatusOr<bool> LcTrieIpList::lookupAddressInTrie(const Instance& address) const {
  if (address.type() != Type::Ip) {
