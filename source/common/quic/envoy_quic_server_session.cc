...
void EnvoyQuicServerSession::OnTlsHandshakeComplete() {
  ...
  // `requiresClientCertificate()` so a client cert presented to a chain that
...
quic::QuicSSLConfig EnvoyQuicServerSession::GetSSLConfig() const {
  ...
  if (position_.has_value()) {
    ...
    // 0-RTT is disabled when a client certificate is required because early
    // data is replayable and would bypass client certificate validation.
