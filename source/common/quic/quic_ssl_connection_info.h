...
//
// QUIC peer certificates live in the BoringSSL `CRYPTO_BUFFER` stack rather
// than on the `SSL` object's X509 cache, so the methods that fetch peer-cert
// fields are overridden to read from `SSL_get0_peer_certificates` and decode
// the leaf on demand.
...
public:
  ...
  bool peerCertificatePresented() const override {
    ...
    return cert_stack != nullptr && sk_CRYPTO_BUFFER_num(cert_stack) > 0;
  }
  ...
  // QUICHE only surfaces the validated leaf, so the issuer-of-leaf accessors
  // remain unimplemented. The XFCC-relevant subject and chain accessors above
  // cover the common consumers.
  ...
  // Raw PEM accessors are not implemented for QUIC. The URL-encoded variants
  // below cover the XFCC use case.
  ...
  const std::string& sni() const override {
    return getCachedCertificateValue<std::string>(&cached_sni_, [this]() -> std::string {
      ...
      const char* server_name = SSL_get_servername(ssl_conn, TLSEXT_NAMETYPE_host_name);
...
private:
  // Caches the result of `extractor`. An empty cached value is treated as
  // "not yet populated" so a value materialised before the handshake completes
  ...
  template <typename U> static bool isCachedValueEmpty(const absl::optional<U>& v) {
    return !v.has_value();
  }
  static bool isCachedValueEmpty(const Ssl::ParsedX509NamePtr& v) { return v == nullptr; }

  // Returns the peer certificate stack from the SSL connection, or nullptr if
  // unavailable.
  ...
  // Returns the leaf peer certificate as a non-owning `CRYPTO_BUFFER`, or
