...
// Owns the WebTransport stream adapters created for one session. Adapters are held for the life of
// the session so consumers like the relay keep stable references, and a closed stream's adapter
// stays as an inert no-op rather than being erased. The list is freed when the session closes, so
// per-session memory is bounded by the session lifetime.
...
private:
  ...
  public:
    explicit Visitor(EnvoyQuicWebTransportStream& stream) : stream_(&stream) {}
    ~Visitor() override;
