...
// Adapts a `vendored` QUICHE client WebTransportHttp3 session to the Envoy
// Http::WebTransportSession interface so the router upstream can relay a session without including
// QUICHE headers. The bridge and the QUICHE session share one owner, the EnvoyQuicClientStream. The
// bridge is a derived-class member so it destructs before the base-class QUICHE session that owns
// the visitor. The destructor detaches the visitor and the visitor null checks the bridge, so
// neither outlives the other unsafely. The bridge installs its visitor on construction because the
// client initiates the session rather than accepting one.
