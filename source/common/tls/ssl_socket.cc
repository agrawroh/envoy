...
Network::IoResult SslSocket::doWrite(Buffer::Instance& write_buffer, bool end_stream) {
  ...
  if (bytes_to_retry_) {
    ...
    bytes_to_write = std::min(write_buffer.length(), static_cast<uint64_t>(data_chunk_send_limit_));
  }
  ...
  while (bytes_to_write > 0) {
    ...
    if (rc > 0) {
      ...
      bytes_to_write =
          std::min(write_buffer.length(), static_cast<uint64_t>(data_chunk_send_limit_));
    } else {
